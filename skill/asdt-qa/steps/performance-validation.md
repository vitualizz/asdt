...
- `pm/handoff` — arrives as an `### INPUT {topic_key}` block. Extract: ONLY the measurable NFR targets from `constraints` (each one's dimension, target value, and measurement method).

**DEGRADATION**: if `pm/handoff` is UNRESOLVED or carries no NFR target, set `gate_verdict` to `no-target` — NEVER a silent `go` — and note `ASSUMED:` in open_items.

## Context budget
test-plan summary + the NFR targets: max 1,200 tokens.
...
3. Roll up the gate: `gate_verdict` is `go` ONLY if every target passes; otherwise `no-go`.
   When no target arrived the roll-up is `no-target`, never `go` — see DEGRADATION in Inputs.
...
payload:
  validations:
    - dimension: ""              # the NFR axis being validated
      target: ""                 # the desired value from pm/handoff constraints
      measured_or_planned: ""    # the planned/measured value from the test-plan
      measurement_method: ""     # HOW it is measured
      verdict: "pass | fail"
      note: ""
  gate_verdict: "go | no-go | no-target"   # no-target (NOT a silent go) when no NFR target arrived
