...
Recommended specialists:
  {specialist name} — {one-line rationale}
    {Tailored Workflow block — format specified in §9.2}

  {specialist name} — {one-line rationale}
    {Tailored Workflow block — format specified in §9.2}

  Security — {one-line rationale}
    {Tailored Workflow block, Security variant — format specified in §9.2}
...
Every Tailored Workflow block written into this format follows the single canonical specification in §9.2 — including Security's variant, which is gated by the independent risk-surface axis rather than by complexity. When `risk_surface` is assessed as `none`, Security MUST NOT appear in the auto-invoked specialist list, but the routing plan MUST still explicitly surface the line `Security — risk_surface: none; not auto-invoked (available on demand via /asdt-security)` so it is never silently dropped.
...
✝ — no §9.1 / §9.1b keyword matched on that axis for that request; the tier shown is the documented outcome of the ONE clarifying question §9.1 / §9.1b prescribe for exactly that case. Every unmarked tier is computed directly from a keyword match.
...
Give each specialist exactly one `## Tailored Workflow` block, written in the canonical format specified in §9.2, carrying the step list for that specialist's assessed tier. A specialist appears at most ONCE in a run order, with a single tier.
...
**Batch the gates**: this section, §9.1, and §9.1b each define a clarifying question, and a single request can leave more than one of them unresolved. When that happens, ask every unresolved question TOGETHER in one turn — one numbered list, one stop, one wait — never one round trip per gate. Only genuinely unresolved gates are asked; a gate resolved by a keyword match is never raised.
...
Authority: each specialist's workflow.yaml owns step identity, execution mode, agent type, and model; each specialist's ## Orchestration Plan in its SKILL.md owns the tier→step lists; the §9.2 per-specialist table is a derived cache of both and never overrides them.
...
> **Parity check**: specialist registration is mirrored by hand in exactly 3 places — the §5 Specialist Registry, this §9.2 per-specialist table (plus its inline-steps list), and `internal/installer/assets/agents-template.md`'s ASDT Specialists table. Keep all 3 in sync when adding or renaming a specialist. You are not the only guard: the repo's automated drift check (`TestRegistryDrift` in `internal/installer/registry_drift_test.go`) derives the canonical roster from the `workflow.yaml` files and fails the build when any of the 3 sites disagrees, and it also asserts the inline-steps region byte-for-byte against the generator.

**Tailored Workflow block format — CANONICAL.** This is the one specification of the block; §6 and §8 reference it rather than restating it.
