...
func TestInstallOne_SiblingLayout(t *testing.T) {
	...
	results := installer.InstallWithModels(assistants, provider, siblingFS, "", nil, installer.InstallOptions{})
...
func TestInstallOne_ProductionShapedSiblingLayout(t *testing.T) {
	...
	results := installer.InstallWithModels(assistants, provider, productionShapedFS, "", nil, installer.InstallOptions{})
	if results[0].Err != nil {
		t.Fatalf("install failed: %v", results[0].Err)
	}
...
func TestInstall_SuccessForTwoAssistants(t *testing.T) {
	...
	results := installer.InstallWithModels(assistants, provider, testFS, "", nil, installer.InstallOptions{})
...
func TestInstall_Idempotent(t *testing.T) {
	...
	results1 := installer.InstallWithModels(assistants, provider, testFS, "", nil, installer.InstallOptions{})
	if results1[0].Err != nil {
		t.Fatalf("first install failed: %v", results1[0].Err)
	}

	results2 := installer.InstallWithModels(assistants, provider, testFS, "", nil, installer.InstallOptions{})
...
func TestInstall_AgentGenFailureIsolation(t *testing.T) {
	...
	results := installer.InstallWithModels(assistants, provider, agentEnabledFS, "", nil, installer.InstallOptions{})
...
func TestInstall_ReinstallPreservesPersonaAndSetsAgentTypes(t *testing.T) {
	...
	results := installer.InstallWithModels([]installer.AssistantDescriptor{assistant}, provider, agentEnabledFS, "", nil, installer.InstallOptions{})
	if results[0].Err != nil {
...
func TestInstall_WritesLanguageToAllAssistantsMeta(t *testing.T) {
	...
	results := installer.InstallWithModels(assistants, provider, agentEnabledFS, "es-ES", nil, installer.InstallOptions{})
	for i, r := range results {
		if r.Err != nil {
...
func TestInstall_ReinstallPreservesLocale(t *testing.T) {
	...
	results := installer.InstallWithModels([]installer.AssistantDescriptor{assistant}, provider, agentEnabledFS, "es-ES", nil, installer.InstallOptions{})
	if results[0].Err != nil {
		t.Fatalf("first install failed: %v", results[0].Err)
	}
...
func TestInstall_PrunesStaleFilesOnUpgrade(t *testing.T) {
	...
	results := installer.InstallWithModels([]installer.AssistantDescriptor{assistant}, provider, pruneV1FS, "", nil, installer.InstallOptions{})
	if results[0].Err != nil {
		t.Fatalf("v1 install failed: %v", results[0].Err)
	}
	checkFile(t, filepath.Join(root, "sample", "old", "extra.md"))

	results = installer.InstallWithModels([]installer.AssistantDescriptor{assistant}, provider, pruneV2FS, "", nil, installer.InstallOptions{})
	if results[0].Err != nil {
...
func TestInstall_MetaPreservedFieldsSurvivePruneManifest(t *testing.T) {
	...
	results := installer.InstallWithModels([]installer.AssistantDescriptor{assistant}, provider, pruneV2FS, "es", nil, installer.InstallOptions{})
	if results[0].Err != nil {
		t.Fatalf("first install failed: %v", results[0].Err)
	}
